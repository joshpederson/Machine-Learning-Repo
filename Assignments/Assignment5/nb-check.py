...
#print(X_test.shape)
cld = DecisionTreeClassifier(max_depth= 3)
cld.fit(X_train, y_train)

print(f"Train Score: {cld.score(X_train, y_train):.3f}")
print(f"Test Score: {cld.score(X_test, y_test):.3f}")

cm = confusion_matrix(y_test, cld.predict(X_test), normalize="true")
disp_cm = ConfusionMatrixDisplay(cm, display_labels=cld.classes_)
disp_cm.plot()
plt.show()