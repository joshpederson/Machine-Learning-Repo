...
#Set model to a Support Vector Classifier using radial basis function kernel
clf = SVC(C=1.0, gamma=1.0, kernel='rbf')

#Setting C and gamma to various numbers for training
#logspace: returns "num" amount of samples from start to stop exponentially
parameters = {'C': np.logspace(-1, 1, base=10, num=3), 'gamma': np.logspace(-2, 1, base=10, num=4)}
...
print(score_clf[['param_C', 'param_gamma', 'mean_test_score', 'rank_test_score']])

#Get the best performing C and gamma
...
clf.C = optC
optGamma = round((score_clf.nsmallest(1, 'rank_test_score')).iloc[0]['param_gamma'], 2)
clf.gamma = optGamma

print()
print(f"----Final Testing W/ Optimal C: {clf.C} and Optimal gamma: {clf.gamma}----")
